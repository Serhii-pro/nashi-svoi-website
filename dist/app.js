...
<section class="panel volunteers"><h2 class="eyebrow">ВОЛОНТЕРИ, ЯКІ НАС ПІДТРИМУЮТЬ</h2>${[0, 1].map((_, i) => `<a href="${i === 0 ? "https://www.instagram.com/bronyasvoih_official" : "https://www.instagram.com/help.for.orphans"}" target="_blank" rel="noopener noreferrer" class="volunteer ${i ? "tone-yellow" : ""}" aria-label="Instagram ${i === 0 ? "bronyasvoih_official" : "help.for.orphans"}"><span class="avatar">${icon("person")}</span><div><strong>${i === 0 ? "bronyasvoih_official" : "help.for.orphans"}</strong><p>напрям допомоги</p></div><span class="icon-button" aria-hidden="true">${icon("instagram")}</span></a>`).join("")}</section></div>`;
const back = () =>
